...
BEGIN_MESSAGE_MAP(CChatDlg, CDialogEx)
	ON_WM_PAINT()
	ON_WM_QUERYDRAGICON()
	ON_BN_CLICKED(IDC_ADD_BTN, &CChatDlg::OnBnClickedAddBtn)
END_MESSAGE_MAP()
...
void CChatDlg::OnBnClickedAddBtn()
{
	CString str;
	...
	int index = 0;
	//목록의 끝 위치에 강제적으로 추가, 현재 추가한 위치값이 반환
	if (str != "") {
		index = m_chat_list.InsertString(-1, str);
		//현재 추가한 위치를 자동 선택
